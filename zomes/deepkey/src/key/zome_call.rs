...
pub enum KeyState {
    // Key anchor.
    Valid(SignedHeaderHash),
    Invalidated(SignedHeaderHash),
    NotFound,
}
...
fn key_state(input: KeyStateInput) -> ExternResult<KeyState> {
    Ok(match get_details(entry_hash(Key::from(input.key))?, GetOptions::latest())? {
        Some(details) => {
            match details {
                Details::Entry(entry_details) => {
                    ...
                    if entry_details.updates.len() > 0 {
                        KeyState::Invalidated(entry_details.updates[0])
                    }
                    // A delete exists.
                    else if entry_details.deletes.len() > 0 {
                        KeyState::Invalidated(entry_details.deletes[0])
                    }
                    // No updates or deletes so this create is still valid.
                    else if entry_details.headers.len() > 0 {
                        KeyState::Valid(entry_details.headers[0])
                    }
