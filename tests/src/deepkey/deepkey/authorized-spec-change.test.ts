...
async function sampleAuthorizedSpecChange(
  cell: CallableCell,
...
export async function createAuthorizedSpecChange(
  cell: CallableCell,
  authorizedSpecChange = undefined
): Promise<Record> {
...
test.skip("create AuthorizedSpecChange", async (t) => {
...
test.skip("create and read AuthorizedSpecChange", async (t) => {
  await runScenario(async (scenario) => {
    // Construct proper paths for your app.
    // This assumes app bundle created by the `hc app pack` command.
    const testAppPath = process.cwd() + "/../workdir/dk-scaffold.happ"
    ...
    // Alice creates a AuthorizedSpecChange
    const record: Record = await createAuthorizedSpecChange(
      alice.cells[0],
      sample
    )
    expect(record).toBeTruthy()
