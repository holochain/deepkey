...
async function sampleDeviceInviteAcceptance(cell: CallableCell, partialDeviceInviteAcceptance = {}) {
    return {
        ...{
...
export async function createDeviceInviteAcceptance(cell: CallableCell, deviceInviteAcceptance = undefined): Promise<Record> {
    return cell.callZome({
      zome_name: "deepkey",
      fn_name: "create_device_invite_acceptance",
      payload: deviceInviteAcceptance || await sampleDeviceInviteAcceptance(cell),
    });
}

test('create DeviceInviteAcceptance', { concurrency: 1 }, async t => {
...
test('create and read DeviceInviteAcceptance', { concurrency: 1 }, async t => {
  await runScenario(async scenario => {
    // Construct proper paths for your app.
    // This assumes app bundle created by the `hc app pack` command.
    const testAppPath = process.cwd() + '/../workdir/dk-scaffold.happ';
    ...
    // Alice creates a DeviceInviteAcceptance
    const record: Record = await createDeviceInviteAcceptance(alice.cells[0], sample);
    assert.ok(record);
